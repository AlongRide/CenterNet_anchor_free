...
class Debugger(object):
  ...
  def show_all_imgs(self, pause=False, time=0):
    print('debugger.py show_all_imgs')
    if not self.ipynb:
      print(' if not self.ipynb')
      for i, v in self.imgs.items():
        cv2.imshow('{}'.format(i), v)
      if cv2.waitKey(0 if pause else 1) == 27:
        import sys
        sys.exit(0)
      cv2.imwrite("/home/cidi/py3torch/CenterNet/test_result/examples.jpg",v)
    else:
      self.ax = None
      nImgs = len(self.imgs)
      ...
      for i, (k, v) in enumerate(self.imgs.items()):
        ...
        if len(v.shape) == 3:
          ...
          self.plt.imshow(v)
      #self.plt.savefig("/home/cidi/py3torch/CenterNet/examples.jpg")
      self.plt.show()
...
coco_class_name = ['traffic', 'sign', 'car', 'motor', 'bike','bus',
